  ...
  if [[ $BALANCEBOX_STATUS -eq 1 ]]; then
    case "$1" in
      start)
        execute \
          --title \
          "${fg[blue]}You're logging in now...${reset_color}" \
          "vagrant ssh $BALANCEBOX_ID"
      ;;
      halt)
        execute \
          --title \
          "${fg[blue]}We are shutting down the ${BALANCEBOX_MACHINE}..." \
      ...
      save)
        execute \
          --title \
          "${fg[blue]}We are suspending the ${BALANCEBOX_MACHINE}..." \
      ...
      reload)
        execute \
          --title \
          "${fg[blue]}We are restarting the ${BALANCEBOX_MACHINE}..." \
    ...
    case "$1" in
      start)
        execute \
          --title \
          "${fg[blue]}We are booting the ${BALANCEBOX_MACHINE}, please be patient..." \
          "vagrant reload $BALANCEBOX_ID &> /dev/null"
